...
'''
        ...
        NOTE:
                To be able to use the I2C on the pine you need to download Enable Pullup from pine64
                And download i2ctools, python-smbus

                EnablePullup -> http://wiki.pine64.org/images/d/d8/EnableI2cPullup.tar.gz
...
class PWN_DRIVER:
    ...
    def servo(self, channel=0, on=0, off=150):
        try:
                self.bus.write_byte_data(self.DRIVER_ADDR, self.LED0_ON_L+4*channel, on & 0xFF)
                self.bus.write_byte_data(self.DRIVER_ADDR, self.LED0_ON_H+4*channel, on >> 8)
                self.bus.write_byte_data(self.DRIVER_ADDR, self.LED0_OFF_L+4*channel, off & 0xFF)
                self.bus.write_byte_data(self.DRIVER_ADDR, self.LED0_OFF_H+4*channel, off >> 8)
        except IOError:
                print("Channel: " + str(channel) + " gave errors on self.DRIVER ADDR: " + str(str(self.DRIVER_ADDR)))
